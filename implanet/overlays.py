...
def _polyline_segments(uvz: np.ndarray, visible: np.ndarray):
    ...
    Returns `(xs, ys)` — two parallel lists of 1-D ndarrays, one entry
    per contiguous visible piece. The boundary crossing is interpolated
    so each polyline ends exactly on the limb (u^2 + v^2 = 1).
    """
    n = len(uvz)
    xs: List[np.ndarray] = []
    ys: List[np.ndarray] = []
    ...
    for i in range(n):
        if visible[i]:
            cur_x.append(uvz[i, 0])
            cur_y.append(uvz[i, 1])
        if i + 1 < n and visible[i] != visible[i + 1]:
            ...
            if abs(denom) > 1e-12:
                ...
                cur_x.append(uvz[i, 0] + t * (uvz[i + 1, 0] - uvz[i, 0]))
                cur_y.append(uvz[i, 1] + t * (uvz[i + 1, 1] - uvz[i, 1]))
            if len(cur_x) >= 2:
                xs.append(np.asarray(cur_x, dtype=np.float64))
                ys.append(np.asarray(cur_y, dtype=np.float64))
            cur_x, cur_y = [], []
    if len(cur_x) >= 2:
        xs.append(np.asarray(cur_x, dtype=np.float64))
        ys.append(np.asarray(cur_y, dtype=np.float64))
...
def graticule_segments(
    ...
    a tuple ``(xs, ys)`` of two parallel lists of 1-D ndarrays; entry
    ``i`` of each list is one contiguous visible piece of a great/small
    circle in unit-disk coordinates.
    ...
    twelve meridians:
        ...
        >>> pxs, pys = g["parallels"]
        >>> len(pxs), len(g["meridians"][0])
        (5, 12)
        >>> pxs[0].shape                     # 1-D array of u-coords
        (181,)
    ...
    Overlay on a matplotlib axes (after `imshow(extent=(-1,1,-1,1))`):
        ...
        ...     ax.plot(x, y, "k--", lw=0.7, alpha=0.55)
        >>> mxs, mys = g["meridians"]
        >>> for x, y in zip(mxs, mys):
        ...     ax.plot(x, y, "k--", lw=0.7, alpha=0.55)
    """
    right, up_axis, forward = camera_basis(view_direction, up)

    p_xs: List[np.ndarray] = []
    p_ys: List[np.ndarray] = []
    ...
    m_xs: List[np.ndarray] = []
    m_ys: List[np.ndarray] = []
...
def terminator_segments(
    ...
    Returns
    -------
    (xs, ys) : tuple of two lists
        Parallel lists of 1-D ndarrays — entry ``i`` of each list is one
        contiguous visible arc in unit-disk (u, v) coordinates. Plot
        with ``for x, y in zip(xs, ys): ax.plot(x, y, …)``.
...
def limb_circle(samples: int = 360):
    """Return (x, y) — two 1-D ndarrays tracing the unit limb circle.

    Examples
    --------
    Draw the planet outline on top of a rendered disk:

        >>> x, y = limb_circle(samples=180)
        >>> x.shape, y.shape
        ((181,), (181,))
...
def flatmap_terminator(
    ...
    Returns the great circle ``{P : P · sun_unit = 0}`` as a tuple
    ``(xs, ys)`` of two parallel lists of 1-D ndarrays — entry ``i`` of
    each list is one polyline in (lon_deg, lat_deg) coordinates,
    suitable for plotting on a (-180, 180, -90, 90)-extent matplotlib
    axes. The curve is split wherever it wraps around the ±180° seam.
    ...
    --------
        ...
        >>> xs, ys = flatmap_terminator(sun_direction=(1, 0, 0.5))
        >>> for x, y in zip(xs, ys):
        ...     ax.plot(x, y, "w--", lw=1.0)
    """
    s = np.asarray(sun_direction, dtype=np.float64)
    n = np.linalg.norm(s)
    if n == 0.0:
    ...
    for a, b in zip(starts, ends):
        if b - a >= 2:
            xs.append(lon[a:b].copy())
            ys.append(lat[a:b].copy())
    return xs, ys
