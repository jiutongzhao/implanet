...
1. **`projection.py`** — pure geometry. `camera_basis()` builds an orthonormal (right, up, forward) triplet by re-orthogonalizing the `up` hint against `forward`. `orthographic_rays()` returns `(H, W, 3)` points on the near hemisphere of the unit sphere plus a disk mask; off-disk pixels are NaN. `perspective_rays()` is the finite-distance counterpart: camera at `C = -distance*forward` (distance in planet radii, >1), each pixel ray-traced into the unit sphere. Framing is **silhouette-normalized** — the apparent outline maps to unit radius exactly as the ortho disk does, so `margin`/extent/`limb_circle` keep their meaning; a nearer camera just sees a smaller visible cap (`arccos(1/distance)`) with more foreshortening. As `distance→∞` it converges to `orthographic_rays`. `sphere_to_uv()` converts sphere points to equirectangular texture coordinates with `lon0` controlling the longitude of the texture's left edge. `resolve_view(name, up=None)` maps the string presets (`"x"`/`"-x"`/`"y"`/`"-y"`/`"z"`/`"-z"` and the plane-named aliases `"xy"`/`"xz"`/`"yz"` + their `-` variants) to explicit `(view_direction, up)` pairs — used by `render_disk`/`render_info`/`plot_disk` so callers can pass `"yz"` instead of `(-1, 0, 0)`. Polar presets supply an in-plane up so `camera_basis` doesn't trip on a parallel-axis degeneracy.
2. **`render.py`** — `render_disk()` glues those primitives to a bilinear texture sampler. All `render_*` accept the `texture` as an ndarray, PIL.Image, **a str/Path** (opened via Pillow by conventional file type; non-`L`/`RGB`/`RGBA` modes coerced to RGB), **or a bare body name** like `"Mars"` (routed through `get_texture()` — handled in `_open_texture` / `_as_texture_array`; path-shaped strings always win). `view_direction` and `up` route through `resolve_view`, so the preset names work everywhere. The sampler **wraps in u** (so meridian seams stay continuous) and **clamps in v** (so poles don't read garbage). Optional Lambertian shading uses `shade = ambient + (1 - ambient) * max(0, P · sun_unit)`. A `distance` argument (default `None` = orthographic, unchanged) switches on `perspective_rays`; overlays (`graticule_segments`, `disk_terminator`) and `plot_disk` take the same `distance` so the vector layers stay registered with the raster. The entire H×W work is vectorized — no per-pixel Python. **Returns just the rendered image** — a uint8 ndarray, row 0 = top. The disk occupies `[-1, +1]` planet radii on both axes; callers plotting with matplotlib use `extent=(-margin, +margin, -margin, +margin)`. `render_flatmap()` is the equirectangular-output counterpart that re-shades a texture in place without orthographic projection (returns a PIL.Image, or an array via `return_array=True`). `render_info()` mirrors the disk signature and returns a structured dict + a one-line caption for figure attributions.
3. **`overlays.py`** — vector overlays in unit-disk coordinates (u² + v² ≤ 1, visible hemisphere only): `graticule_segments`, `limb_circle`, `subobserver_point`, `disk_terminator` (the great circle `{P : P · sun_unit = 0}`, clipped at the limb), and `flatmap_terminator` (the lat/lon-space counterpart). Each returns plain `(xs, ys)` arrays so callers compose their own matplotlib plots from `render_disk` + overlays.
4. **`plot.py`** — a thin matplotlib convenience: `plot_disk(texture, view_direction="yz", sun_direction=None, ..., ax=None)` calls `render_disk` for the raster, then `limb_circle` / `graticule_segments` / `disk_terminator` / `subobserver_point` to overlay limb, graticule, terminator (only when a sun is given), and the sub-observer cross. Returns `(fig, ax)`. Per-overlay `*_kwargs` dicts merge over the defaults so callers can restyle each layer; `show_axes=True` swaps the clean image-plate axes for publication-style planet-radii ticks. This is the only layer that imports matplotlib at module load (matplotlib is in `dependencies`, not optional).
5. **`ephemeris.py`** — thin SPICE wrappers. `sun_direction(body, utc)` and `sub_solar_point(body, utc)` go through J2000 + `pxform` rather than asking SPICE to resolve `IAU_<body>` directly — our `de440s.bsp` only carries planet barycenters, so moon SPK centers are unavailable; for moons, we use the parent barycenter as the position origin (sub-arcsecond error vs. AU baselines). Body table is in `_BODY_INFO`. `__init__.py` imports this module behind a `try/except ImportError`, so the package keeps working without `spiceypy`.
...
- `Layer 1` (`projection.py`), `Layer 2` (`render.py`), and `Layer 3` (`overlays.py`) only depend on numpy + Pillow. `plot.py` (Layer 4) is the only place below `ephemeris.py` that touches matplotlib; don't import matplotlib from Layers 1–3, and don't import spiceypy below `ephemeris.py` — the optional-dependency boundary on `spiceypy` is load-bearing.
